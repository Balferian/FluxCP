...
if (count($_POST)) {
    ...
    elseif (!Flux::config('AllowUserInPassword') &&
        stripos(str_replace(" ", "", $account->name),preg_replace('/[^a-z]/i','',$newPassword)) !== false) {
    ...
    elseif (Flux::config('PasswordMinSymbol') > 0 && preg_match_all('/[^A-Za-z0-9]/', $newPassword, $matches) < $passwordMinSymbol) {
		...
		if(Flux::config('MasterAccountPasswordHash') == "bcrypt") {
			...
			$currentPassword = Flux::hashPassword($currentPassword, Flux::config('MasterAccountPasswordHash'));
			$newPassword     = Flux::hashPassword($newPassword, Flux::config('MasterAccountPasswordHash'));
			...
			if ($currentPassword != $account->currentPassword) {
				...
				if ($sth->execute(array($newPassword, $session->account->id))) {
					...
					$sql .= "VALUES (?, ?, ?, ?, NOW())";
					$sth  = $server->connection->getStatement($sql);
					$sth->execute(array($session->account->id, $currentPassword, $newPassword, $_SERVER['REMOTE_ADDR']));
