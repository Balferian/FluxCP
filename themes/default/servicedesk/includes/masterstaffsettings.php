	...
	<table class="horizontal-table" width="100%"> 
		...
		<?php foreach($stafflist as $trow):?>
			<tr >
				...
				<td><?php echo $this->linkToMasterAccount($trow->user_id, $trow->name) ?></td>
				...
				<td>
					...
					<?php if($session->isMine($trow->account_id)): ?>
						<a href="<?php echo $this->url('servicedesk', 'staffsettings', array('option' => 'alerttoggle', 'staffid' => $trow->account_name, 'cur' => $trow->emailalerts))?>" ><i>(toggle)</i></a>
					<?php endif ?>
					</td>
				<?php if(isset($staffsess) && $staffsess->team>'1'): ?>
				<td><a href="<?php echo $this->url('servicedesk', 'staffsettings', array('option' => 'delete', 'staffid' => $trow->account_name))?>" >Delete</a></td>
				<?php endif ?>
			</tr>
		<?php endforeach;?>
		</tbody>
...
<form action="<?php echo $this->urlWithQs ?>" method="post" class="input_fill">
	<table class="horizontal-table" width="100%">
		...
		<tr>
			<td colspan="4">
			<button type="submit">Add Staff</button></td>
		</tr>
    </table>
</form>
