...
class Flux_VipShop {
	 ...
	 */
	public function add($categoryID, $cost, $quantity, $info)
	{
		$db    = $this->server->charMapDatabase;
		$table = Flux::config('FluxTables.VipServiceShopTable');
		$sql   = "INSERT INTO $db.$table (category, quantity, cost, info, create_date) VALUES (?, ?, ?, ?, NOW())";
		$sth   = $this->server->connection->getStatement($sql);
		$res   = $sth->execute(array($categoryID, $quantity, $cost, $info));
	 ...
	 */
	public function edit($shopItemID, $categoryID = null, $cost = null, $quantity = null, $info = null)
	{
		$catQ = '';
		$crdQ = '';
		$qtyQ = '';
		...
		$db    = $this->server->charMapDatabase;
		$table = Flux::config('FluxTables.VipServiceShopTable');
		$sql   = "UPDATE $db.$table SET $catQ $crdQ $qtyQ $infQ WHERE id = ?";
		$sth   = $this->server->connection->getStatement($sql);
	...
	public function getItem($shopItemID)
	{
		...
		$shop  = Flux::config('FluxTables.VipServiceShopTable');
		$col   = "$shop.id AS shop_item_id, $shop.category AS shop_item_category, $shop.cost AS shop_item_cost, $shop.quantity AS shop_item_qty, ";
		$col  .= "$shop.category AS shop_category, $shop.info AS shop_item_info";
		$sql   = "SELECT $col FROM $db.$shop WHERE $shop.id = ? ORDER BY shop_category, shop_item_qty ASC";
		$sth   = $this->server->connection->getStatement($sql);
	...
	public function getItems($paginator, $categoryID = null)
	{
		...
		$sql   = $paginator->getSQL("SELECT $col FROM $db.$shop $sqlpartial ORDER BY shop_category, shop_item_qty ASC");
		$sth   = $this->server->connection->getStatement($sql);
